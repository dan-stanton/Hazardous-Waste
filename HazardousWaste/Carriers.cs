...
namespace HazardousWaste
{
    public partial class Carriers : Form
    {
        ...
        {
            string query;
            if (!Deleted.Checked) query = "SELECT Name, Address1, Address2, Address3, Address4, Postcode, Permit FROM Carriers WHERE CONCAT(Name, Address1, Address2, Address3, Address4, Postcode, Permit) LIKE '%" + valueToSearch + "%' AND Deleted = 0";
            else query = "SELECT Name, Address1, Address2, Address3, Address4, Postcode, Permit FROM Carriers WHERE CONCAT(Name, Address1, Address2, Address3, Address4, Postcode, Permit) LIKE '%" + valueToSearch + "%'";
        ...
        {
            using (EditCarrier form = new EditCarrier())
            {
